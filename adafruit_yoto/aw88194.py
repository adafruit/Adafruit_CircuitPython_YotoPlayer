...
class AW88194:
    ...
    """RCV_GAIN - receiver mode AMP_NORM_V
     - 0: AMP_NORM_V=4.5 1: AMP_NORM_V=5 2: AMP_NORM_V=5.5 3: AMP_NORM_V=5.5"""
    spk_gain = RWBits(3, _REG_SYSCTRL, 12, register_width=_REG_WIDTH, lsb_first=False)
    """SPK_GAIN - 0: AMP_NORM_V=7 1: AMP_NORM_V=8 2: AMP_NORM_V=10
     3: AMP_NORM_V=12 4: AMP_NORM_V=14 6: AMP_NORM_V=16"""

    # I2SCTRL @ 0x05
    i2s_sample_rate = RWBits(4, _REG_I2SCTRL, 0, register_width=_REG_WIDTH, lsb_first=False)
    """I2SSR - sample rate
    0: 8 kHz, 1: 11.025 kHz, 2: 12 kHz, 3: 16 kHz, 4: 22.05 kHz, 5: 24 kHz,
    6: 32 kHz, 7: 44.1 kHz, 8: 48 kHz, 9: 96 kHz, 10~15: Reserved"""
    ...
    """I2SMD - interface mode
     - 0: Philips standard, 1: MSB justified, 2: LSB justified, 3: Reserved"""
    ...
    """VOL - volume control from 0 to -96dB (0 being max volume)
     - bits 7-4: increments of -6dB
     - bits 3:0: increments of -0.5dB"""

    # BSTCTRL1 - boost control register 1 @ 0x60
    boost_ath = RWBits(6, _REG_BSTCTRL1, 0, register_width=_REG_WIDTH, lsb_first=False)
    """BST_ATH - smart boost attack threshold
    - when the signal is above the threshold,
     the voltage of VBST will be raised higher than VDD in smart boost mode"""
    boost_rth = RWBits(6, _REG_BSTCTRL1, 8, register_width=_REG_WIDTH, lsb_first=False)
    """BST_RTH - smart boost release threshold
    - when the signal is below the threshold,
     the voltage of VBST will not be raised higher than VDD in SBM"""
    ...
    """BST_TDEG - smart boost small signal level detection deglitch time
    - 0: 1.33 ms 1: 2.66 ms 2: 5.32 ms 3: 21.30 ms 4: 85.20 ms 5: 340.79 ms 6: 1.363 s 7: 2.73 s"""
    boost_mode = RWBits(3, _REG_BSTCTRL2, 4, register_width=_REG_WIDTH, lsb_first=False)
    """BST_MODE - smart boost mode
     - 0: transparent 1: force boost 5: smart boost 1 6: smart boost 2"""
    ...
    def enable(self, poll_timeout_s=0.5):
        """Enables I2S input and unmutes the amplifier,
         then polls SYSST for PLL lock and boost ready state
